...
import { LayoutDashboard, CalendarCheck, FlaskConical, MessageSquare, BarChart3, Settings, ClipboardList, LogOut, Menu, ChevronDown, Package, FileText } from 'lucide-react';
      ...
      <aside className={`w-[260px] min-h-screen gradient-sidebar text-primary-foreground fixed top-0 left-0 z-50 flex flex-col transition-transform duration-300 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="p-5 border-b border-primary-foreground/10">
          <div className="flex items-center gap-3">
            ...
            <div><div className="font-heading font-bold text-sm leading-tight">CvSU Research Center</div><div className="text-[0.7rem] text-primary-foreground/50">Admin Panel</div></div>
          </div>
        </div>
      ...
      <div className={`flex-1 flex flex-col min-h-screen transition-[margin] duration-300 ${sidebarOpen ? 'lg:ml-[260px]' : 'lg:ml-0'}`}>
        <header className="bg-card border-b border-border px-6 py-3.5 flex justify-between items-center sticky top-0 z-30 shadow-sm">
          <div className="flex items-center gap-3">
            ...
            <h1 className="font-heading text-base font-bold text-foreground">{menuItems.find((m) => m.to === location.pathname)?.label || 'Admin'}</h1>
          </div>
          <div className="flex items-center gap-3">
            <NotificationBell reservationsLink="/admin/reservations" equipmentLink="/admin/equipment" />
              ...
              <>
                  ...
                  <div className="absolute right-0 top-[calc(100%+0.5rem)] z-50 w-56 bg-card border border-border rounded-2xl shadow-xl overflow-hidden animate-fade-up p-2">
                    ...
                    <Link to="/admin/settings" onClick={() => setMenuOpen(false)}
                      className={`flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm font-semibold no-underline transition-colors mb-1 ${location.pathname === '/admin/settings' ? 'bg-primary/10 text-primary' : 'text-foreground hover:bg-muted'}`}>
